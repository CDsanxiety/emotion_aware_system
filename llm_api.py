...
def _ensure_full_chain_payload(
    ...
    if isinstance(ex, dict):
        result["execution"] = {
            ...
            "action": ex.get("action", result.get("action", "none")),
...
def _mock_reply_memory_recall(prefix: str, user_text: str) -> dict:
    ...
    if _wants_transcript_recall(t):
        ...
        return {"emotion": "平静", "action": "none", "reply": reply}
...
def call_llm(
    ...
    try:
        _to = float(os.getenv("LLM_REQUEST_TIMEOUT_SEC", "12"))
